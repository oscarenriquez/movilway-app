...
public class TipoCampanaHelper extends ServicioHelper {
	...
	public void comboBoxTipoCampana (HttpServletRequest req, HttpServletResponse resp, int key) throws ServletException, IOException {		
		try{
			setDefaultValues(req, key);
			JSONObject result = new JSONObject();
				...
				if(permiso){						
					try{
						...
						for(TipoCampana tipoCampana : listaTipoCampana) {
							...
							jsObj.put("ID", tipoCampana.getTipocampanaId());
							jsObj.put("DESCRIPCION", tipoCampana.getDescripcion().toUpperCase());
							lista.add(jsObj);
						}
						...
						getAlerta().enviarAlerta("comboBoxTipoCampana", ie, getUsuarioBean(),  EMAIL);
						ie.printStackTrace();
						msg = DISABLED_BD;
						isSuccess = false;
					} catch (Exception e){
						getAlerta().enviarAlerta("comboBoxTipoCampana", e, getUsuarioBean(),  EMAIL);
						e.printStackTrace();
						msg = DISABLED_BD;
						isSuccess = false;
					}						
			...
			e.printStackTrace();
			getAlerta().enviarAlerta("comboBoxTipoCampana", e, getUsuarioBean(), ServicioHelper.EMAIL);
		}
	}
}
