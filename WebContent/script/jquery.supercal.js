...
(function(factory) {
	...
	var defaults = {
		todayButton: false,		// Boton dia actual
	...
	var shortDays = ['Domingo', 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado'];
	...
	$.fn.supercal = function(method) {
		...
		var pMethods = {
			...
			drawHeader: function(date, options) {
				...
				if(options.fnPrevMes != null){
					btn1.on("click",options.fnPrevMes);
				}
				...
				if(options.fnPrevMes != null){
					btn2.on("click",options.fnNextMes);
				}
				...
				$('<span />')
					...
					.append('<input type="hidden" name="mes" id="mes" value="' + monthNames[date.getMonth()] + '">')
					.append('<input type="hidden" name="anioServ" id="anioServ" value="' + date.getFullYear() + '">')
					.appendTo(header);
			...
			drawMonth: function(date, options) {
				...
				for(var row = 0; row < 6; row++) {
					...
					for(var col = 0; col < 7; col++) {
						...
						var nomID ='d'+cell.estado+cell.displayNumber; //id a la celda de tabla
