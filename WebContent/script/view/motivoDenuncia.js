...
$.ajaxSetup({
 ...
 function showEditMotivoD(motivo) { 
		idMotivo = null;
		var param = new Array();
		
		param.push({"name" : "key", "value" : 182});		 
		...
		$.post("./Session", param, function(data){
			if(data.permiso){
				if(data.isSuccess){ 
					idMotivo = motivo;  
					$("#descripcion").val(data.aData.motivo);
					 
					$("#conte_gestion").show();
					$("#conte_listado").hide();
					$("#conteBtnEdit").show();
					$("#conteBtnGuardar").hide();
					$("#titleGestion").html('Editar Motivo');
...
function clickButton(accion) {   
	...
	var mensaje = "";  
		 ...
		 if(!vParam(descripcion)){  
			 noty({text: "Ingrese motivo de denuncia", type:'error', timeout:7000});  
				return false; 
		} 
		...
		if(accion === 1){
			param.push({"name" : "key", "value" : 180});	
			mensaje = "Esta seguro de crear el motivo de denuncia: "+descripcion+" ?";
		}else {
			
			 if(vParam(idMotivo)){  
				 param.push({"name" : "key", "value" : 181});
				 param.push({"name" : "fuenteId", "value" : idMotivo});
			 ...
			 mensaje = "Esta seguro de Editar el motivo de denuncia: "+descripcion+" ?";
		}
			    
		param.push({"name" : "motivo", "value" : descripcion});  
			...
			if (e) { 
				...
				appWait.showPleaseWait();
					...
					$.post("./Session", param, function(data){
						...
						appWait.hidePleaseWait();  
						setTimeout(buildFormBack(178), 300); 							
					});
			  } else {
			return false;   
			}
...
function motivList(){
	try{		
		if(tableMotivo !== null){
			tableMotivo.fnDestroy();
			tableMotivo = null;
		}
	 
		
		tableMotivo =	$('#tabla_motivo').dataTable({  
			"bProcessing": true, 
	        "bServerSide": true,
	        "bJQueryUI": true,
	        "bDestroy": true,
	        ...
	        "fnServerData": function(sSource, aoData, fnCallback, oSettings) {
	            aoData.push({"name": "key","value": 179}); 
	            $.ajax({
	                "dataType": 'json',
	                "type": "POST",
	                "url": sSource,
